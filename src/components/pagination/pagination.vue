<template>
  <ul class="lte-pagination pagination no-margin" :class="['pagination' + size]">
    <li :class="{disabled: value <=1}" @click="setPageIndex(1, value <= 1)"><a href="javascript:;">{{firstText}}</a></li>
    ...
    <li :class="{disabled: value >= pageCount}" @click="setPageIndex(pageCount, value >= pageCount)"><a href="javascript:;">{{lastText}}</a></li>
  </ul>
</template>
<script>
  export default {
